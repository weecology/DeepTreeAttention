...
def test_TreeData_setup(config, ROOT):
    ...
    dm = data.TreeData(config=config, csv_file=csv_file, data_dir="{}/tests/data/processed/".format(ROOT), debug=True) 
    dm.setup()  
...
def test_TreeDataset(config, ROOT):
    ...
    image = inputs["RGB"]
    assert image.shape == (3, config["RGB_image_size"], config["RGB_image_size"])
