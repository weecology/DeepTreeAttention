...
def test_empty_plot(rgb_path, plot_data):
    ...
    if hasattr(deepforest_model, "load_model"):
        deepforest_model.load_model(model_name="weecology/deepforest-tree")
    else:
...
def test_generate_crops(tmpdir, ROOT, rgb_path, sample_crowns):
    ...
    per_tree = annotations.groupby("individual").size()
    assert per_tree.eq(2).all()
