...
def test_predict_dataloader(config, m, dm, experiment, ROOT):
    df = m.predict_dataloader(dm.val_dataloader(), experiment = None)
...
def test_evaluate_crowns(config, experiment, m, dm, ROOT):
    ...
    df = m.evaluate_crowns(val_dataloader = dm.val_dataloader(), train_dataloader = dm.train_dataloader(), logger=None)
