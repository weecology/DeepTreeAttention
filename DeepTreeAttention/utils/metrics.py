...
def site_confusion(y_true, y_pred, site_lists):
    ...
    Args: 
        ...
        site_lists: dict of site labels for each string label taxonID -> sites
    Returns:
...
def genus_confusion(y_true, y_pred):
    ...
    for index, value in enumerate(y_pred):
        #If not correctly predicted
        if not value == y_true[index]:
            true_sci_name = scientific_dict[y_true[index]].split().str.get(0)
            pred_sci_name = scientific_dict[y_pred[index]]
