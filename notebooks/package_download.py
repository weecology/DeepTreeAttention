...
def clean_up(path):
    ...
    try:
        a = a.drop(columns=["conifer_la","broadleaf_","dominant_c","oak_label","ensembleTa","ens_label"])
        a = a.rename(columns=
                     {"broadlea_1":"bleaf_score",
...
for site in ["BART"]:
    ...
    predictions = glob.glob("/blue/ewhite/b.weinstein/DeepTreeAttention/results/predictions/BART/{}/*.shp".format(basename), recursive=True)
    futures = client.map(clean_up, predictions)
    wait(futures)
    
zipfilename = "/blue/ewhite/b.weinstein/DeepTreeAttention/results/predictions/{}/{}/{}.zip".format(site, basename, site)
#remove old
try:
...
for site in species_model_paths:
    #Download model
    p = subprocess.Popen("scp hpg:{} /Users/benweinstein/Dropbox/Weecology/Species/SpeciesMaps/snapshots/".format(species_model_paths[site]), shell=True)
    p.wait()
    ...
    if not os.path.exists("/Users/benweinstein/Dropbox/Weecology/Species/SpeciesMaps/{}.zip".format(site)):
        #Copy zip locally
        print(site)
        p = subprocess.Popen("scp hpg:{} /Users/benweinstein/Dropbox/Weecology/Species/SpeciesMaps/".format(zipfilename), shell=True)
        p.wait()
